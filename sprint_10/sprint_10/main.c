...
# Projeto final da disciplina de LASD - Laboratorio de Arquitetura de Sistemas Digitais

#	Explicacao do codigo:
#	Sistema de um carro eletrico, nele podemos visualizar o nivel de bateria, a sua respectiva temperatura, o funcionamento do motor DC
#	um sensor sonar para medir distancias proximas e acionar os freios, aplicacao direta do PWM, ADC, botoes acionadores, o power train
#	e todo fluxo de dados sendo disponibilizado no display de 7-segmentos a velocidade atual do carro, e no display ssd1306 todos os dados
#	referentes ao carro como o diametro do pneu, senso de distancia, RPM, nivel da bateria e temperatura, estado atual do motor P, D ou E,
#	e a distancia percorrida pelo carro.
#
#	Alem disso tudo ja mencionado nesse ultima sprint foi implementado um pisca alerta automatico, em que sua funcao eh identificar alguma
#	situacao adversa e ligeiramente acender o pisca alerta como sinal de aviso para outros veiculos proximos, evitando algum acidente.
#	Alem do pisca alerta foi implementado um farol alto acionado sempre que o carro estiver em mvimento. Ambas aplicacoes sao acionadas
#	automaticamente. Mas foi implementado também um metodo para acionar ambas fora da condicao habitual por meio da serial, assim o condutor
#	do veicula podera usar tais ferramentas caso necessite de um faro alto sem que o carro esteja em movimento, ou um pisca alerta sem que
#	o carro esteja em situacao adversa como pneu furado ou qualquer outra situacao que o impossibilite de se mover.
#
#	Quando acionado o pisca alerta, tambem eh informado no display ssd1306 com o simbulo de exclamacao '!', sinalizando situacao de alerta,
#	caso contrario se estiver apenas de farol alto, um outro simbulo eh mostrado simbolizando carro andando '>', e quando ambas LEDS tanto
#	o pisca alerta como farol alto estiverem ligado, sera mostrado o simbolo '?' referente ao uso desconhecido dos farois.
*/
...
uint8_t flag_5ms = 0, flag_500ms = 0, flag_250ms = 0, flag_over_flow = 0;
...
ISR(USART_RX_vect)
{
	...
	if(recebido == 'z')		// se recebido == z, eh zerado o valor da temperatura maxima na eeprom
	{
	...
	if(recebido == 't')		// se recebido == t, sera transmitido pela usart o valor maximo da temperatura
	{
		for (int i = 9; i < 13; i++)
		{
	...
	if(recebido == 'e')		// se recebido == e, sera acionado tanto o pisca alerta como o farol alto do carro
	{
		PORTC ^= 0b01001000;
	}
}
...
ISR(TIMER0_COMPA_vect)
{
	...
	if((tempo_ms % 5) == 0)		//true a cada 5ms
		flag_5ms = 1;
	if((tempo_ms % 250) == 0)	//true a cada 250ms
		flag_250ms = 1;
